...
| **resume-service** | 8004 | File-based (no DB) | Processes uploaded resumes. Uses NLP to parse candidate data (skills, experience) and generate tailored questions. No persistent DB; uses local file storage. |
...
| **feedback-service** | 8010 | SQLite | Collects interview responses and scores. Generates feedback reports. May call LLM (OpenAI/Gemini) to draft feedback summaries. Stores scores and feedback in SQLite. |
...
4. Optionally invokes LLM (e.g. OpenAI GPT-4 or Google Gemini) to refine the selected follow-up
5. Generates custom questions if none are a good fit
6. Novel follow-ups generated by LLM are inserted back into Pinecone for future reuse

**RAG Implementation:** This process (query embeddings → retrieve from vector DB → optional LLM rewriter) constitutes the live retrieval and generation step of RAG, mirroring typical two-phase RAG architecture.
...
| **LLM** | OpenAI GPT-4 / Google Gemini | Generating feedback reports and optional question refinement |
...
- API keys for OpenAI, AssemblyAI, database connection strings, OAuth credentials
